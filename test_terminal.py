...
class TestTerminal(unittest.TestCase):
    ...
    def test_read_numeric_ub(self, input):
        with self.assertRaises(InputException):
            im.read_numeric('Input', bl_int=True, num_lb=0, num_ub=10, bl_inc_lb=True, bl_inc_ub=True)

    @patch('terminal.InputManager.read_line', return_value=10)
    def test_read_numeric_ub_2(self, input):
        with self.assertRaises(InputException):
            im.read_numeric('Input', bl_int=True, num_lb=0, num_ub=10, bl_inc_lb=True, bl_inc_ub=False)
    ...
    def test_read_numeric_lb_2(self, input):
        with self.assertRaises(InputException):
            im.read_numeric('Input', bl_int=True, num_lb=0, num_ub=10, bl_inc_lb=False, bl_inc_ub=True)
